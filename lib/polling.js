...
const minimumPollInterval = 1000*60*10;
const daysBackToHistory = 3; // days
const pollFullUpdateDelay = 300; // sec
/*
...
TradeOfferManager.prototype.doPoll = function(doFullUpdate) {
	...
	if (Date.now() - this._lastPollFullUpdate >= 1000*pollFullUpdateDelay || doFullUpdate) {
		//fullUpdate = true; // we don't need full history, so comment it
		this._lastPollFullUpdate = Date.now();
		offersSince = new Date((Date.now()/1000)-60*60*24*daysBackToHistory); // cut offers history according to daysBackToHistory setting
	}
