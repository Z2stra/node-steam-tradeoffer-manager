...
function TradeOfferManager(options) {
	...
	this.apiKey = null;
	this.apiKeyReserve  = options.api; // get reserve api key
	this.steamID = null;
...
TradeOfferManager.prototype._checkApiKey = function(callback) {
	...
	this._community.getWebApiKey(this._domain, (err, key) => {
		if (err) {
			if (err.message.indexOf('API key')>-1){ // set reserve api if we got an error
				key = this.apiKeyReserve;
				callback(err);	// send callback to show what we got an error (but dont leave the function)
			} 
			else {
				callback(err);
				return;
